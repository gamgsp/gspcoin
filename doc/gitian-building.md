...
*Setup instructions for a Gitian build of Gspcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gspcoin
...
- [Building Gspcoin Core](#building-gspcoin-core)
...
Clone the git repositories for Gspcoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/gspcoin/gspcoin
...
in the Gspcoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Gspcoin Core
----------------

To build Gspcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gspcoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gspcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gspcoin/gspcoin
...
For example:
```bash
URL=https://github.com/crowning-/gspcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gspcoin=${COMMIT} --url gspcoin=${URL} ../gspcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gspcoin=${COMMIT} --url gspcoin=${URL} ../gspcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gspcoin=${COMMIT} --url gspcoin=${URL} ../gspcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gspcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gspcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gspcoin
SIGPATH=/some/root/path/gspcoin-detached-sigs

./bin/gbuild --url gspcoin=${BTCPATH},signature=${SIGPATH} ../gspcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gspcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gspcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gspcoin-osx-build.assert
```
...
[gspcoin/gitian.sigs](https://github.com/gspcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
