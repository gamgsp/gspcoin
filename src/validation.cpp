...
#if defined(NDEBUG)
# error "Gspcoin Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Gspcoin
...
{
	double nSubsidy = 2525;      // (declaring the reward variable and its original/default amount)
    ...
    if (nPrevHeight < 10) {
        ...
        nSubsidy = 2925;     
    } else if ((nPrevHeight > 500000) && (nPrevHeight < 600000)) {
        nSubsidy = 3025;      
    } else if ((nPrevHeight > 600000) && (nPrevHeight < 700000)) {
        nSubsidy = 3125;     
    } else if ((nPrevHeight > 700000) && (nPrevHeight < 800000)) {
        nSubsidy = 3225;  
    } else if ((nPrevHeight > 800000) && (nPrevHeight < 900000)) {
        nSubsidy = 3325;      
    } else if ((nPrevHeight > 900000) && (nPrevHeight < 1000000)) {
        nSubsidy = 3425;     
    } else if ((nPrevHeight > 1000000) && (nPrevHeight < 1100000)) {
        nSubsidy = 3425;    
    } else if ((nPrevHeight > 1100000) && (nPrevHeight < 1200000)) {
        nSubsidy = 3325;     
    } else if ((nPrevHeight > 1200000) && (nPrevHeight < 1300000)) {
        nSubsidy = 3225;  
    } else if ((nPrevHeight > 1300000) && (nPrevHeight < 1400000)) {
        nSubsidy = 3125;      
    } else if ((nPrevHeight > 1400000) && (nPrevHeight < 1500000)) {
        nSubsidy = 3025;     
    } else if ((nPrevHeight > 1500000) && (nPrevHeight < 1600000)) {
        nSubsidy = 2925;     
    } else if ((nPrevHeight > 1600000) && (nPrevHeight < 1700000)) {
        nSubsidy = 2825;     
    } else if ((nPrevHeight > 1700000) && (nPrevHeight < 1800000)) {
        nSubsidy = 2725;     
    } else if ((nPrevHeight > 1800000) && (nPrevHeight < 1900000)) {
        nSubsidy = 2625;     
    } else if ((nPrevHeight > 1900000) && (nPrevHeight < 2000000)) {
        nSubsidy = 2525;     
...
static int64_t nTimeGspcoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    /// GSPCOIN: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(GSPCOIN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // GSPCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(GSPCOIN): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(GSPCOIN): spork is off, skipping transaction locking checks\n");
    }
    ...
    // GSPCOIN : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(GSPCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(GSPCOIN): couldn't find smartnode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeGspcoinSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Dash specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeGspcoinSpecific * MICRO, nTimeGspcoinSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(GSPC): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
