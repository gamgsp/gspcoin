...
const char * const BITCOIN_CONF_FILENAME = "gspcoin.conf";
const char * const BITCOIN_PID_FILENAME = "gspcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "gspcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GspCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GspCoin
    // Mac: ~/Library/Application Support/GspCoin
    // Unix: ~/.gspcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GspCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GspCoin";
#else
    // Unix
    return pathRet / ".gspcoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty gspcoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Gspcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Gspcoin Core developers";
    }
