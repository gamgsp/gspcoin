...
void static GspcoinMiner(const CChainParams& chainparams)
{
    LogPrintf("GspcoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gspcoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("GspcoinMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("GspcoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("GspcoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("GspcoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("GspcoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateGspcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&GspcoinMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
