...
PACKAGE_VERSION=$($SRC_PATH/src/gspcoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/gspcoind $DEB_BIN
cp $SRC_PATH/src/gspcoin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/gspcoin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/gspcoind.1 $DEB_MAN
cp $SRC_DOC/man/gspcoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/gspcoin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/gspcoind.bash-completion $DEB_CMP/gspcoind
cp $SRC_PATH/contrib/gspcoin-cli.bash-completion $DEB_CMP/gspcoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/gspcoind.1
gzip --best -n $DEB_MAN/gspcoin-cli.1
gzip --best -n $DEB_MAN/gspcoin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/gspcoind $DEB_BIN/gspcoin-cli
