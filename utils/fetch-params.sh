...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/GspcoinParams"
else
    PARAMS_DIR="$HOME/.gspcoin-params"
fi
...
DOWNLOAD_URL="https://github.com/Raptor3um/gspcoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with GSPC_DISABLE_SOMETHING=1
GSPC_DISABLE_WGET="${GSPC_DISABLE_WGET:-}"
#GSPC_DISABLE_IPFS="${GSPC_DISABLE_IPFS:-}"
GSPC_DISABLE_CURL="${GSPC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$GSPC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$GSPC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$GSPC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Gspcoin parameters!
...
main() {
    ...
    cat <<EOF
GSPC - fetch-params.sh

This script will fetch the Gspcoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Gspcoin SNARK parameters. Note that it is
