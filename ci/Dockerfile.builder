...
# gspcoin_hash
RUN git clone https://github.com/gspcoin/gspcoin_hash
RUN cd gspcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} gspcoin
RUN useradd -u ${USER_ID} -g gspcoin -s /bin/bash -m -d /gspcoin gspcoin
...
RUN mkdir /gspcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /gspcoin-src && \
